    ...
    <div className='flex items-center justify-between py-5 font-medium'>
        ...
        <div className='flex items-center gap-6'>
                  ...
                  <>
                    ...
                    <div className='group-hover:block hidden absolute dropdown-menu right-0 pt-4 z-50'>
                        <div className='flex flex-col gap-2 w-36 py-3 px-5 bg-white text-gray-500 rounded shadow-lg border border-gray-200'>
                            <Link to='/profile' className='cursor-pointer hover:text-black transition-colors'>My Profile</Link>
                            <Link to='/orders' className='cursor-pointer hover:text-black transition-colors'>Orders</Link>
            ...
            <Link to='/wishlist' className='relative'>
                ...
                {getWishlistCount() > 0 && (
                    <p className='absolute right-[-5px] bottom-[-5px] w-4 text-center leading-4 bg-red-500 text-white aspect-square rounded-full text-[8px]'>{getWishlistCount()}</p>
                )}
            </Link>
        ...
        <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all z-50 ${visible ? 'w-full' : 'w-0'}`}>
            <div className='flex flex-col text-gray-600'>
                ...
                <NavLink onClick={()=>setVisible(false)} className='py-2 pl-6 border' to='/cart'> CART ({getCartCount()})</NavLink>
                <NavLink onClick={()=>setVisible(false)} className='py-2 pl-6 border' to='/wishlist'> WISHLIST ({getWishlistCount()})</NavLink>
                {isAuthenticated ? (
                  <>
                    ...
                    <button onClick={() => { logout(); setVisible(false); }} className='py-2 pl-6 border text-left w-full'> LOGOUT</button>
                  </>
                ) : (
